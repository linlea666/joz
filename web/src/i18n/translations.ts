...
export const translations = {
  en: {
    ...
    discord: {
      ...
      tokenPlaceholder: 'Paste your Discord token',
      ...
      save: 'Save',
      saving: 'Saving...',
      saved: 'Discord configuration saved',
      saveFailed: 'Failed to save Discord configuration',
      testOk: 'Connected as',
      testFailed: 'Connection test failed',
      clearToken: 'Clear token',
      clearConfirm:
        'Clear the Discord token? All copy-trading traders will stop receiving signals.',
      cleared: 'Discord token cleared',
      clearFailed: 'Failed to clear token',
      testChannelLabel: 'Test Channel ID',
      channelOk: 'Channel readable',
      channelFailed: 'Channel test failed',
      channelStatus: 'Watched Channels',
      channelError: 'error',
    ...
    copytrade: {
      ...
      typeHint:
        'Follows trade signals from a Discord channel. Requires the global Discord token in Settings. Type cannot be changed after creation.',
      configTitle: 'Copy Trading Configuration',
      ...
      channelNotesGenerate: 'AI draft from history',
      channelNotesGenerating: 'Drafting…',
      channelNotesGenerated: 'Draft filled in — review and save to apply',
      channelNotesGenerateFailed: 'Failed to draft channel profile',
      riskMode: 'Position Sizing Mode',
      riskByLoss: 'By Risk (loss)',
      riskPercent: '% of Equity',
      riskFixed: 'Fixed Margin',
      riskByLossHint:
        'Quantity = risk amount ÷ |entry − stop loss|. Each stopped-out trade loses about the risk amount.',
      riskPercentHint:
        'Margin = equity × percentage. Enter the percentage (e.g. 5 = 5%).',
      riskFixedHint: 'Fixed margin in USD per trade.',
      riskAmountUsd: 'Risk Amount (USD)',
      riskAmountPct: 'Equity Percentage (%)',
      maxNotional: 'Max Position Notional (USD)',
      majorLeverage: 'BTC/ETH Leverage',
      altLeverage: 'Altcoin Leverage',
      maxPositions: 'Max Open Positions',
      tpRatios: 'Default TP Allocation',
      tpRatiosHint:
        'Position % allocated to TP1,TP2,TP3 when the author gives multiple targets without ratios (max 3 levels, sum ≤ 100).',
      majorOffset: 'BTC/ETH Price Offset Threshold (%)',
      altOffset: 'Altcoin Price Offset Threshold (%)',
      offsetHint:
        "A market at or better than the author's price always enters at market. A worse price within the threshold may still enter at market; beyond it a limit order rests at the author's price. 0 allows no adverse difference, even for an author's market signal. Defaults on a new trader do not change saved traders.",
      ...
      interpretationProfile: 'Interpretation profile',
      profileDefault: 'General',
      profileHint: 'TYLER applies its management phrases only to this follower. Ambiguous partial profit closes 50% of the remaining position; add-only conditions require a confirmed fill.',
      ...
      retryWaiting: 'Waiting for model retry',
      dupProtection: 'Duplicate Open Protection',
      paused: 'Paused (block new opens)',
      on: 'ON',
      off: 'OFF',
      // Log view
      logTitle: 'Copy Trading Log',
      tabEvents: 'Events',
      tabSignals: 'Signals',
      tabContexts: 'Active Trades',
      tabAIStats: 'AI Latency',
      tabReplay: 'Recognition Replay',
      replayHint:
        'Re-runs AI interpretation over stored channel history. Dry run only: no orders are placed and nothing is written to stats — use it to verify recognition accuracy before going live.',
      ...
      replayStats:
        'Would execute {execute} · skip {skip} · invalid {invalid} · error {error} · avg LLM {avg}s',
      replayImageCap: '(max 3 sent)',
      showIO: 'View AI I/O',
      hideIO: 'Hide AI I/O',
      loadingIO: 'Loading AI interaction…',
      noAIRun: 'No AI run recorded for this signal.',
      ...
      decisionMarketPrice: 'Market price at decision',
      adverseDeviation: 'Adverse difference',
      appliedThreshold: 'Threshold used',
      ...
      ioParsed: 'Parsed JSON',
      ioImages: 'images attached',
      replayVerdictExecute: 'Would Execute',
      replayVerdictSkip: 'Skip',
      replayVerdictInvalid: 'Invalid',
      replayVerdictError: 'Error',
      noEvents: 'No events yet',
      noSignals: 'No signals yet',
      noContexts: 'No active trades',
      noStats: 'No AI runs recorded',
      colTime: 'Time',
      colEvent: 'Event',
      colMessage: 'Message',
      colAction: 'Action',
  ...
  zh: {
    ...
    discord: {
      ...
      notConfigured: '未配置',
      tokenLabel: 'Discord Token',
      tokenPlaceholder: '粘贴你的 Discord Token',
      tokenKeepPlaceholder: '留空则保留当前 Token',
      tokenHint:
      ...
      cleared: 'Discord Token 已清除',
      clearFailed: '清除失败',
      testChannelLabel: '测试频道 ID',
      channelOk: '频道可读取',
      channelFailed: '频道测试失败',
      channelStatus: '监听中的频道',
      channelError: '异常',
      subscribers: '个订阅者',
    ...
    copytrade: {
      ...
      channelNotesHint:
        '只作为识别用的背景知识，不会改输出契约。改完后到跟单日志「识别回放」重跑即可对比效果。',
      channelNotesTemplate: '插入模板',
      channelNotesGenerate: '根据历史 AI 起草',
      channelNotesGenerating: '起草中…',
      channelNotesGenerated: '草稿已填入，请核对后保存才会生效',
      channelNotesGenerateFailed: '频道画像生成失败',
      riskMode: '仓位计算模式',
      riskByLoss: '以损定量',
      riskPercent: '资金百分比',
      riskFixed: '固定保证金',
      ...
      riskAmountPct: '资金百分比（%）',
      maxNotional: '单仓最大名义价值（USD）',
      majorLeverage: 'BTC/ETH 杠杆',
      altLeverage: '山寨币杠杆',
      maxPositions: '最大同时持仓数',
      tpRatios: '默认止盈分配',
      tpRatiosHint:
        '博主给出多个止盈目标但未指定比例时，按 TP1,TP2,TP3 分配的仓位百分比（最多 3 档，总和 ≤ 100）。',
      majorOffset: 'BTC/ETH 市价偏移阈值（%）',
      altOffset: '山寨币市价偏移阈值（%）',
      offsetHint:
        '市价优于或等于博主价时始终直接市价进场；比博主价差但在阈值内也可市价进场，超出阈值则按博主价挂限价单等待。填 0 不容许任何不利价差，博主明确要求市价也受此限制。新建页面的默认值不会修改已保存交易员的配置。',
      limitToMarket: '阈值内限价改市价',
      parseImages: '解析图片（AI 视觉）',
      autoBreakeven: 'TP1 后止损移至保本',
      interpretationProfile: '消息解读规则',
      profileDefault: '通用',
      profileHint: 'TYLER 话术规则仅作用于当前跟单实例。模糊止盈口令减仓剩余仓位的 50%；有补仓限定的指令须确认补仓成交。',
      entryPolicy: '进场策略',
      entryLegacy: '沿用原进场规则',
      ...
      tabReplay: '识别回放',
      replayHint:
        '对已存储的频道历史消息重新做 AI 识别。纯演练：不会下单、不写入统计数据，用于上线前检验识别准确度。',
      replayCount: '回放条数',
      replayStart: '开始回放',
      replayRunning: '回放中',
      replayEmpty: '暂无回放结果，选择条数后点击「开始回放」。',
      replayFailed: '回放启动失败',
      replayProfileHint:
        '在交易员卡片里改完频道画像后，回到这里重跑回放，对比识别是否更好。',
      replayStats:
        '会执行 {execute} · 跳过 {skip} · 无效 {invalid} · 错误 {error} · 平均 LLM {avg}s',
      replayImageCap: '（最多发送 3 张）',
      showIO: '查看 AI 交互',
      hideIO: '收起交互',
      loadingIO: '正在加载 AI 交互…',
      noAIRun: '该信号没有 AI 调用记录。',
      ...
      executionLoadFailed: '执行详情加载失败，请收起后重新展开重试。',
      noEntryDecision: '没有记录进场决策快照，无法还原当时价差。',
      entryDecision: '进场决策',
      authorOrderType: '博主订单意图',
      referencePrice: '参考价（区间取中点）',
      decisionMarketPrice: '决策时市价',
      adverseDeviation: '不利价差',
      appliedThreshold: '实际使用阈值',
      plannedOrderType: '计划订单类型',
      ...
      ioParsed: '解析后的 JSON',
      ioImages: '附带图片',
      replayVerdictExecute: '会执行',
      replayVerdictSkip: '跳过',
      replayVerdictInvalid: '无效',
      replayVerdictError: '错误',
      noEvents: '暂无事件',
      noSignals: '暂无信号',
      noContexts: '暂无进行中的交易',
      noStats: '暂无 AI 调用记录',
